  ...
  useEffect(() => {
    ...
    const logDistances = () => {
      if (pastRef.current && presentRef.current && futureRef.current) {
        ...
        if (comp1Ref.current) comp1Ref.current.style.opacity = op1;
        if (gradient1.current) gradient1.current.style.opacity = op1;
        if (grid1.current) grid1.current.style.opacity = op1;

        // set opacity for the second and third sections
        if (comp2Ref.current) comp2Ref.current.style.opacity = op2;
        if (gradient2.current) gradient2.current.style.opacity = op2;
        if (grid2.current) grid2.current.style.opacity = op2;

        if (comp3Ref.current) comp3Ref.current.style.opacity = op3;
        if (gradient3.current) gradient3.current.style.opacity = op3;
        if (grid3.current) grid3.current.style.opacity = op3;

        // Display hello svg animation once user scrolls down to the past section
        if (pastRef.current && pastRef.current.getBoundingClientRect().top <= 350) {
          if (svgRef.current) svgRef.current.style.display = 'block';
          if (svgRef2.current) svgRef2.current.style.display = 'block';
        } else {
          if (svgRef.current) svgRef.current.style.display = 'none';
          if (svgRef2.current) svgRef2.current.style.display = 'none';
        }
      }
    };
  ...
  <path stroke="#4D79A5" stroke-width="4" stroke-linecap="round" stroke-linejoin="round"/>
            ...
            When I’m not knee-deep in coding, you’ll find me devouring novels, playing badminton, or losing myself in RPGs (Elden Ring, anyone?). Travel’s my constant. Every new city teaches me how to see the world differently.
              ...
              <div className="experience-about-wrapper">
                ...
                <div className="resume-wrapper">
                  <a href="/sifar-resume.pdf" target="_blank" rel="noopener noreferrer" className="resume-link">
            ...
            <div className="left">
              <div className="image-container">
                <img ref={comp1Ref} className="computer-1" src="/computers/computer-1.webp" alt="" />
                <svg
                  ref={svgRef}
                  className="draw-svg"
                  width="150"
...
<path stroke="#4D79A5" stroke-width="4" stroke-linecap="round" stroke-linejoin="round"/>
                ...
                <img ref={comp2Ref} className="computer-2" src="/computers/computer-2.webp" alt="" />
                <img ref={comp3Ref} className="computer-3" src="/computers/computer-3.webp" alt="" />
